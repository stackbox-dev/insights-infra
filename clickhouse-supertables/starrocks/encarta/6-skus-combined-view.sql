-- StarRocks View: encarta_skus_combined
-- Equivalent to ClickHouse's parameterized encarta_skus_combined({node_id:Int64}) view
--
-- KEY DIFFERENCE FROM CLICKHOUSE:
--   ClickHouse uses parameterized views: encarta_skus_combined(node_id = 123)
--   StarRocks does NOT support parameterized views.
--   Instead, use: SELECT * FROM encarta_skus_combined WHERE node_id = 123
--
-- Override logic: overrides table uses NULL columns, so COALESCE works directly.
--   COALESCE(so.field, sm.field) = use override if not NULL, else use master.
--
-- Usage:
--   -- Get SKU data for a specific warehouse:
--   SELECT * FROM encarta_skus_combined WHERE node_id = 123;
--
--   -- Get only master data (no overrides):
--   SELECT * FROM encarta_skus_combined WHERE node_id IS NULL;
--
--   -- Use in enrichment JOINs (replaces ClickHouse MV enrichment):
--   SELECT pd.*, sku.name, sku.l0_weight
--   FROM wms_pick_drop pd
--   LEFT JOIN encarta_skus_combined sku ON pd.sku_id = sku.sku_id AND sku.node_id = pd.wh_id;

CREATE OR REPLACE VIEW encarta_skus_combined AS
SELECT
    -- SKU Identifiers
    sm.id AS sku_id,
    so.node_id AS node_id,   -- NULL when no override exists for this sku+node combination
