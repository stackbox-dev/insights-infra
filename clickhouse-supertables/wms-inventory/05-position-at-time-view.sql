...
-- Combined data: snapshot + events after snapshot
combined_data AS (
    -- 1. Snapshot data (already aggregated - use as baseline)
    SELECT 
        ...
        -- Quantity change (snapshot is cumulative)
        cumulative_qty as qty_change,
        total_event_count as event_count,
    ...
    WHERE wh_id = (SELECT value FROM param_wh_id)
      ...
      AND (SELECT value FROM latest_snapshot_time) != toDateTime('1970-01-01')
    
    UNION ALL
    
    -- 2. Events after snapshot (individual events - will be aggregated in final step)
    SELECT 
        -- Core inventory identifiers
        wh_id,
        hu_id,
        ...
        -- Quantity change (raw event quantity)
    ...
    WHERE wh_id = (SELECT value FROM param_wh_id)
      AND hu_event_timestamp > (SELECT value FROM latest_snapshot_time)
      AND hu_event_timestamp <= (SELECT value FROM param_target_time)
)

-- Final aggregation: combine snapshot + events
SELECT 
    -- Core inventory identifiers
    wh_id,
    hu_id,
    argMax(hu_code, event_time) as hu_code,
    sku_id,
    uom,
    bucket,
    batch,
    ...
    -- Latest values for enriched fields (using argMax for time-based selection)
...
-- Example usage:
-- SELECT * FROM wms_inventory_position_at_time(wh_id = 4012, target_time = now())