...
if [ -z "$1" ]; then
    ...
    echo "Example: $0 .samadhan-prod.env"
    exit 1
fi
...
# Set TOPIC_PREFIX from KAFKA_ENV if not already set
if [ -z "$TOPIC_PREFIX" ]; then
    TOPIC_PREFIX="$KAFKA_ENV"
fi
...
# OMS tables (all from Debezium public topics)
TOPIC_MAPPINGS='[
...
curl -X PUT http://localhost:8083/connectors/clickhouse-connect-${TOPIC_PREFIX}-oms/config \
...
-d  '{
      ...
      "errors.deadletterqueue.topic.name": "dlq-oms-clickhouse",
