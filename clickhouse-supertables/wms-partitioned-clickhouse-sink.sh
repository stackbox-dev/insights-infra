...
# Define topic to table mappings for WMS Partitioned
TOPIC_MAPPINGS='[
  {"topic": "public.handling_unit", "table": "wms_handling_units"},
  {"topic": "public.trip", "table": "wms_trips"},
  {"topic": "public.invoice", "table": "wms_invoices"},
  {"topic": "public.invoice_line", "table": "wms_invoice_line"}
]'
...
echo "Creating/Updating ClickHouse connector for WMS Partitioned..."
...
curl -X PUT http://localhost:8083/connectors/clickhouse-connect-digitaldc_prod-wms-partitioned/config \
...
if [ $? -eq 0 ]; then
    echo "✅ ClickHouse connector for WMS Partitioned created/updated successfully!"
    ...
    curl -s http://localhost:8083/connectors/clickhouse-connect-digitaldc_prod-wms-partitioned/status | jq .
    
    echo ""
    echo "📋 Useful commands:"
    echo "  # Check connector status"
    echo "  curl -s http://localhost:8083/connectors/clickhouse-connect-digitaldc_prod-wms-partitioned/status | jq ."
    echo ""
    echo "  # Restart connector"
    echo "  curl -X POST http://localhost:8083/connectors/clickhouse-connect-digitaldc_prod-wms-partitioned/restart"
    echo ""
    echo "  # Delete connector"
    echo "  curl -X DELETE http://localhost:8083/connectors/clickhouse-connect-digitaldc_prod-wms-partitioned"
else
    echo "❌ Failed to create/update ClickHouse connector for WMS Partitioned"
    exit 1
fi