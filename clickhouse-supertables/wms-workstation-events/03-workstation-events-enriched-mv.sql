...
SELECT
    ...
    -- nullable columns from staging wrapped with ifNull to prevent MV insert failure
    we.event_type AS event_type,
    we.event_source_id AS event_source_id,
    we.event_timestamp AS event_timestamp,
    ifNull(we.created_at, toDateTime64('1970-01-01 00:00:00', 3)) AS created_at,
    we.wh_id AS wh_id,
    ifNull(we.sku_id, '') AS sku_id,
    ifNull(we.hu_id, '') AS hu_id,
    ifNull(we.hu_code, '') AS hu_code,
    ifNull(we.batch_id, '') AS batch_id,
    ifNull(we.user_id, '') AS user_id,
    ifNull(we.task_id, '') AS task_id,
    ifNull(we.session_id, '') AS session_id,
    ifNull(we.bin_id, '') AS bin_id,
    ifNull(we.primary_quantity, 0) AS primary_quantity,
    ifNull(we.secondary_quantity, 0) AS secondary_quantity,
    ifNull(we.tertiary_quantity, 0) AS tertiary_quantity,
    ifNull(we.price, '') AS price,
    ifNull(we.status_or_bucket, '') AS status_or_bucket,
    ifNull(we.reason, '') AS reason,
    ifNull(we.sub_reason, '') AS sub_reason,
    ifNull(we.deactivated_at, toDateTime64('1970-01-01 00:00:00', 3)) AS deactivated_at,
    ...
    -- All sbm columns are Nullable in samadhan_rw; wrap with ifNull to avoid MV insert failure
    ifNull(sbm.bin_code, '') AS bin_code,
    ifNull(sbm.bin_created_at, toDateTime64('1970-01-01 00:00:00', 3)) AS bin_created_at,
    ifNull(sbm.bin_updated_at, toDateTime64('1970-01-01 00:00:00', 3)) AS bin_updated_at,
    ifNull(sbm.bin_description, '') AS bin_description,
    ifNull(sbm.bin_status, '') AS bin_status,
    ifNull(sbm.bin_hu_id, '') AS bin_hu_id,
    ifNull(sbm.multi_sku, false) AS multi_sku,
    ifNull(sbm.multi_batch, false) AS multi_batch,
    ifNull(sbm.picking_position, 0) AS picking_position,
    ifNull(sbm.putaway_position, 0) AS putaway_position,
    ifNull(sbm.rank, 0) AS bin_rank,
    ifNull(sbm.aisle, '') AS aisle,
    ifNull(sbm.bay, '') AS bay,
    ifNull(sbm.level, '') AS level,
    ifNull(sbm.position, '') AS bin_position,
    ifNull(sbm.depth, '') AS depth,
    ifNull(sbm.bin_type_code, '') AS bin_type_code,
    ifNull(sbm.zone_id, '') AS zone_id,
    ifNull(sbm.zone_code, '') AS zone_code,
    ifNull(sbm.zone_description, '') AS zone_description,
    ifNull(sbm.area_id, '') AS area_id,
    ifNull(sbm.area_code, '') AS area_code,
    ifNull(sbm.area_description, '') AS area_description,
    ifNull(sbm.x1, 0.0) AS x1,
    ifNull(sbm.y1, 0.0) AS y1,
    ifNull(sbm.max_volume_in_cc, 0.0) AS max_volume_in_cc,
    ifNull(sbm.max_weight_in_kg, 0.0) AS max_weight_in_kg,
    ifNull(sbm.pallet_capacity, 0) AS pallet_capacity,
