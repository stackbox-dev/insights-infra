...
A Kubernetes CronJob (`kafka-connector-healthcheck`) runs every 30 minutes in the `kafka` namespace and performs **4 automated checks**, sending Slack alerts when issues are detected:

| # | Check | Alert Condition |
|---|-------|----------------|
| 1 | **Connector Health** | Any connector task goes to `FAILED` state |
| 2 | **Consumer Lag** | Any consumer group lag exceeds `50000` messages |
| 3 | **Topic Size** | Any topic exceeds `100 GB` in size |
| 4 | **PG Replication Slot Lag** | Any Debezium replication slot lag exceeds `1 GB` |
...
|----------|------|-----------|-------------|
| CronJob | `kafka-connector-healthcheck` | `kafka` | Runs every 30 min, all 4 checks |
...
```
CronJob: kafka-connector-healthcheck (*/30 * * * *)
  └── alpine container
        ...
        │     └── If consumerLag > LAG_THRESHOLD (50000) → Slack alert (:warning:)
        │
        ├── CHECK 3: Topic Size
        ...
        │
        └── CHECK 4: PG Replication Slot Lag
              └── psql query on pg_replication_slots WHERE slot_name LIKE 'dbz_%'
              └── pg_wal_lsn_diff(pg_current_wal_lsn(), restart_lsn) > 1 GB
              └── If lag > LAG_THRESHOLD_GB (1 GB) → Slack alert (:warning:)
```
...
| `LAG_THRESHOLD` | Alert when `consumerLag > threshold` | `50000` |
| `TOPIC_SIZE_THRESHOLD_GB` | Alert when topic size > this value in GB | `100` |
| `LAG_THRESHOLD_GB` | Alert when PG replication slot WAL lag > this value in GB | `1` |
...
[2026-03-04 08:00:03] ===== CHECK 2: Consumer Lag (threshold: 50000) =====
...
- **`concurrencyPolicy: Forbid`** — If a job takes longer than 30 minutes, the next scheduled run is skipped. `activeDeadlineSeconds: 300` kills jobs stuck beyond 5 minutes.
