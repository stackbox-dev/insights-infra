...
if [ -z "$CLICKHOUSE_HOSTNAME" ]; then
    echo "Error: CLICKHOUSE_HOSTNAME not set in environment file"
    exit 1
fi
...
CH_PARAMS="--host=$CLICKHOUSE_HOSTNAME"
[ -n "$CLICKHOUSE_HTTP_PORT" ] && CH_PARAMS="$CH_PARAMS --port=$CLICKHOUSE_HTTP_PORT"
[ -n "$CLICKHOUSE_USER" ] && CH_PARAMS="$CH_PARAMS --user=$CLICKHOUSE_USER"
[ -n "$CLICKHOUSE_ADMIN_PASSWORD" ] && CH_PARAMS="$CH_PARAMS --password=$CLICKHOUSE_ADMIN_PASSWORD"
...
echo "ClickHouse Host: $CLICKHOUSE_HOSTNAME"
