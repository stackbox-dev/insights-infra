...
# Flink Monitor Runner Script
# This script sets up the Python environment and runs the Flink monitor
...
for arg in "$@"; do
    if [[ "$arg" == "-h" || "$arg" == "--help" ]]; then
        ...
        setup_venv
        python3 "$SCRIPT_DIR/flink_monitor.py" --help
        exit 0
    fi
done
...
log_info "Starting Flink Monitor..."
if python3 "$SCRIPT_DIR/flink_monitor.py" "$@"; then
