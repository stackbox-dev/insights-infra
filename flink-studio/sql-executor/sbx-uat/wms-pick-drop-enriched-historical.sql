SET 'pipeline.name' = 'WMS Pick Drop Historical Enrichment';
...
-- Source: Historical pick-drop data (bounded stream with processing time)
...
CREATE TABLE tasks (
    ...
    event_time AS COALESCE(`updatedAt`, `createdAt`),
    WATERMARK FOR event_time AS event_time - INTERVAL '5' SECOND,
    proc_time AS PROCTIME()
) WITH (
    'connector' = 'kafka',
    'topic' = 'sbx_uat.wms.public.task',
    'scan.bounded.mode' = 'latest-offset',  -- Make bounded for batch processing
...
-- Create deduplicated views for dimension tables to ensure 1 row per key
...
CREATE VIEW sku_overrides_latest AS
SELECT * FROM (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY sku_id, node_id ORDER BY event_time DESC) as rn
    FROM sku_overrides
) WHERE rn = 1;

CREATE VIEW sku_masters_latest AS
SELECT * FROM (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY id ORDER BY event_time DESC) as rn
    FROM sku_masters
...
CREATE VIEW trip_relation_rekeyed_latest AS
SELECT * FROM (
    SELECT *,
        ROW_NUMBER() OVER (PARTITION BY sessionId, childTripId ORDER BY event_time DESC) as rn
    FROM trip_relation_rekeyed
) WHERE rn = 1;
...
FROM pick_drop_basic pb
    LEFT JOIN sessions_latest s ON pb.session_id = s.id
    LEFT JOIN tasks_latest t ON pb.task_id = t.id
    LEFT JOIN trips_latest lm_trip ON pb.lm_trip_id = lm_trip.id
    LEFT JOIN trip_relation_rekeyed_latest tr ON pb.session_id = tr.sessionId
    AND pb.lm_trip_id = tr.childTripId
    LEFT JOIN trips_latest parent_trip ON tr.parentTripId = parent_trip.id
    LEFT JOIN workers_latest w ON pb.picked_by_worker_id = w.id
    LEFT JOIN handling_units_latest hu ON pb.dropped_inner_hu_id = hu.id
    LEFT JOIN sku_overrides_latest pick_so ON pb.picked_sku_id = pick_so.sku_id
    AND pb.wh_id = pick_so.node_id
    LEFT JOIN sku_masters_latest pick_sm ON pb.picked_sku_id = pick_sm.id
    LEFT JOIN sku_overrides_latest drop_so ON pb.dropped_sku_id = drop_so.sku_id
    AND pb.wh_id = drop_so.node_id
    LEFT JOIN sku_masters_latest drop_sm ON pb.dropped_sku_id = drop_sm.id;
---