...
FROM `sbx-uat.wms.public.pd_pick_item` pi
    LEFT JOIN `sbx-uat.wms.public.pd_pick_drop_mapping` pdm ON pi.id = pdm.`pickItemId`
    LEFT JOIN `sbx-uat.wms.public.pd_drop_item` di ON pdm.`dropItemId` = di.id
    JOIN `sbx-uat.wms.public.task` t ON pi.`taskId` = t.id
    JOIN `sbx-uat.wms.public.session` s ON t.`sessionId` = s.id
    LEFT JOIN `sbx-uat.wms.public.worker` w ON pi.`pickedBy` = w.id
    LEFT JOIN `sbx-uat.wms.public.handling_unit` hu ON di.`droppedInnerHUId` = hu.id
    LEFT JOIN `sbx-uat.wms.public.trip` lm_trip ON pi.`lmTripId` = lm_trip.id
    LEFT JOIN `sbx-uat.wms.public.trip` mm_trip ON pi.`mmTripId` = mm_trip.id
