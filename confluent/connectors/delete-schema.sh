...
#   status=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE -u "$SCHEMA_REGISTRY_AUTH" \
...
for subject in $sorted_subjects; do
  echo -n "Deleting subject: $subject ... "
  status=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE "http://kafka-ui.kafka.api.staging.stackbox.internal/api/clusters/services-1-staging/schemas/$subject")
